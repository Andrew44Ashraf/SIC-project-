...
class Assembler(object):
	...
	@property
	def pure_object_code(self):
		ret=[]
		for i in (self.generated_objects):
			ret.append(i[1][2])
		return ret
...
if __name__ == '__main__':
	datafile="dataset2/sic-opcode"+".txt"
	f=open(datafile,"w")
...
#	f.write("MUL\t20\n")
	...
	for itr in range(0,500000):
		if(itr%100==0):
			print("Iteration: "+str(itr)+"/500000\n")
		gen_code(False,True,min(itr/50000,10))
		a = Assembler(open('intermediate.txt','r'))
		a.assemble()
		g=a.generated_objects
		...
		x="-ins"
		if(x in args):
			for record in a.instructions:
				print record
			print("\n")

		x="-opcode"
		if(x in args):
			for record in a.pure_object_code:
				print record
			print("\n")

		x="-hte"
		if(x in args):
			for record in a.generated_records:
				print record
			print("\n")

	
		for idx in range(1,len(a.instructions)-1):
			record=a.instructions[idx]
			f.write(record+".")
			if(idx!=len(a.instructions)-2):
				f.write(" ")
		f.write("\t")

		for idx in range(0,len(a.pure_object_code)):
			record=a.pure_object_code[idx]
			f.write(record)
			if(idx!=len(a.pure_object_code)-1):
				f.write(" ")
		f.write("\n")
